...
public class Tela4 extends JFrame {
    ...
    // Personalização das cores do JOptionPane (como antes)
    ...
    JComboBox<String> campoCidade = new JComboBox<>(new String[]{"São Paulo", "Guarulhos", "Campinas", "São Bernardo do Campo", "Santo André"});
    JComboBox<String> comboTipo = new JComboBox<>(new String[]{"Tipo 1", "Tipo 2"});
    JComboBox<String> comboEstado = new JComboBox<>(new String[]{"Sem tratamento", "Em tratamento", "Internado", "Óbito"});
    ...
    painel.setBackground(new Color(0xE1F5FE)); // mesmo fundo azul claro
    ...
    if (opcao == JOptionPane.YES_OPTION) { // "Salvar"
        try {
            String url = "jdbc:mysql://localhost:3306/aps";
            String usuario = "root";
            String senha = "APS-2025";
    ...
    // Restaura cores padrão (se quiser)
